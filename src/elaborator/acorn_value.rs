...
impl AcornValue {
    ...
    /// Reassociates curried applications so `f(a)(b)` becomes `f(a, b)`.
    pub fn flatten_applications(&self) -> AcornValue {
        match self {
            AcornValue::Variable(_, _) | AcornValue::Constant(_) | AcornValue::Bool(_) => {
                self.clone()
            }
            AcornValue::Application(app) => {
                let function = app.function.flatten_applications();
                let args: Vec<_> = app
                    .args
                    .iter()
                    .map(|arg| arg.flatten_applications())
                    .collect();
                if let AcornValue::Application(mut nested) = function {
                    nested.args.extend(args);
                    AcornValue::Application(nested)
                } else {
                    AcornValue::Application(FunctionApplication {
                        function: Box::new(function),
                        args,
            ...
            AcornValue::TypeApplication(app) => AcornValue::TypeApplication(TypeApplication {
                function: Box::new(app.function.flatten_applications()),
            ...
            AcornValue::Lambda(args, value) => {
                AcornValue::Lambda(args.clone(), Box::new(value.flatten_applications()))
            }
            AcornValue::Binary(op, left, right) => AcornValue::Binary(
                *op,
                Box::new(left.flatten_applications()),
                Box::new(right.flatten_applications()),
            ),
            AcornValue::Not(value) => AcornValue::Not(Box::new(value.flatten_applications())),
            AcornValue::Try(value, t) => {
                AcornValue::Try(Box::new(value.flatten_applications()), t.clone())
            }
            AcornValue::ForAll(quants, value) => {
                AcornValue::ForAll(quants.clone(), Box::new(value.flatten_applications()))
            }
            AcornValue::Exists(quants, value) => {
            ...
            AcornValue::IfThenElse(cond, if_value, else_value) => AcornValue::IfThenElse(
                ...
                Box::new(else_value.flatten_applications()),
            ),
            AcornValue::Match(scrutinee, cases) => {
                let scrutinee = scrutinee.flatten_applications();
                let cases = cases
                    .iter()
                    .map(|case| MatchCase {
                        new_vars: case.new_vars.clone(),
                        pattern: case.pattern.flatten_applications(),
                        result: case.result.flatten_applications(),
                        constructor_index: case.constructor_index,
                        constructor_total: case.constructor_total,
                    })
                    .collect();
    ...
    /// Eta-reduces lambdas whose bound variables occur only as the trailing application args.
    pub fn reduce_eta(&self, stack_size: AtomId) -> AcornValue {
        match self {
            AcornValue::Variable(_, _) | AcornValue::Constant(_) | AcornValue::Bool(_) => {
                self.clone()
            }
            AcornValue::Application(app) => AcornValue::Application(FunctionApplication {
                function: Box::new(app.function.reduce_eta(stack_size)),
                args: app
                    .args
                    .iter()
                    .map(|arg| arg.reduce_eta(stack_size))
                    .collect(),
            }),
            AcornValue::TypeApplication(app) => AcornValue::TypeApplication(TypeApplication {
                function: Box::new(app.function.reduce_eta(stack_size)),
            ...
            AcornValue::Lambda(args, value) => {
                let new_stack_size = stack_size + args.len() as AtomId;
                let value = value.reduce_eta(new_stack_size);
                AcornValue::try_reduce_eta_lambda(args, &value, stack_size)
                    .unwrap_or_else(|| AcornValue::Lambda(args.clone(), Box::new(value)))
            }
            AcornValue::Binary(op, left, right) => AcornValue::Binary(
                *op,
                Box::new(left.reduce_eta(stack_size)),
                Box::new(right.reduce_eta(stack_size)),
            ),
            AcornValue::Not(value) => AcornValue::Not(Box::new(value.reduce_eta(stack_size))),
            AcornValue::Try(value, t) => {
                AcornValue::Try(Box::new(value.reduce_eta(stack_size)), t.clone())
            }
            AcornValue::ForAll(quants, value) => {
                let new_stack_size = stack_size + quants.len() as AtomId;
                AcornValue::ForAll(quants.clone(), Box::new(value.reduce_eta(new_stack_size)))
            }
            AcornValue::Exists(quants, value) => {
                let new_stack_size = stack_size + quants.len() as AtomId;
            ...
            AcornValue::IfThenElse(cond, if_value, else_value) => AcornValue::IfThenElse(
                ...
                Box::new(else_value.reduce_eta(stack_size)),
            ),
            AcornValue::Match(scrutinee, cases) => {
                let scrutinee = scrutinee.reduce_eta(stack_size);
                let cases = cases
                    .iter()
                    .map(|case| {
                        let new_stack_size = stack_size + case.new_vars.len() as AtomId;
                        MatchCase {
                            new_vars: case.new_vars.clone(),
                            pattern: case.pattern.reduce_eta(new_stack_size),
                            result: case.result.reduce_eta(new_stack_size),
                            constructor_index: case.constructor_index,
                            constructor_total: case.constructor_total,
                        }
                    })
    ...
    fn try_reduce_eta_lambda(
        args: &[AcornType],
        body: &AcornValue,
        stack_size: AtomId,
    ) -> Option<AcornValue> {
        let AcornValue::Application(app) = body else {
            return None;
        };
        if app.args.len() < args.len() {
            return None;
        }

        let first_bound_index = stack_size;
        let end_bound_index = first_bound_index + args.len() as AtomId;
        let split_at = app.args.len() - args.len();
        let (prefix_args, eta_args) = app.args.split_at(split_at);

        for (offset, (expected_type, actual_arg)) in args.iter().zip(eta_args.iter()).enumerate() {
            match actual_arg {
                AcornValue::Variable(index, actual_type)
                    if *index == first_bound_index + offset as AtomId
        ...
        if app
            .function
            .references_stack_range(first_bound_index, end_bound_index)
        {
            return None;
        }
        if prefix_args
            .iter()
            .any(|arg| arg.references_stack_range(first_bound_index, end_bound_index))
        {
            return None;
        }

        Some(AcornValue::apply(
            (*app.function).clone(),
            prefix_args.to_vec(),
        ))
    }

    fn references_stack_range(&self, start: AtomId, end: AtomId) -> bool {
        match self {
            AcornValue::Variable(index, _) => *index >= start && *index < end,
            AcornValue::Application(app) => {
                app.function.references_stack_range(start, end)
                    || app
                        .args
                        .iter()
                        .any(|arg| arg.references_stack_range(start, end))
            }
            AcornValue::TypeApplication(app) => app.function.references_stack_range(start, end),
            ...
            AcornValue::Binary(_, left, right) => {
                left.references_stack_range(start, end) || right.references_stack_range(start, end)
            }
            AcornValue::IfThenElse(cond, if_value, else_value) => {
                cond.references_stack_range(start, end)
                    || if_value.references_stack_range(start, end)
                    || else_value.references_stack_range(start, end)
            }
            AcornValue::Match(scrutinee, cases) => {
                scrutinee.references_stack_range(start, end)
                    || cases.iter().any(|case| {
                        case.pattern.references_stack_range(start, end)
                            || case.result.references_stack_range(start, end)
                    })
            }
