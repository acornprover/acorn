...
1. Check the module in baseline mode:

```bash
cargo run --profile release -- verify MODULENAME --ignore-hash --read-only --fail-fast
```

2. Check the module in feature mode:
...
If feature mode still fails on the same line, inspect that line again and add more detail.

If feature mode now fails on a later line, move to that new line.

## Writing Cache

Stay in `--read-only` mode during the explication loop.

Only write cache when the surrounding workflow explicitly wants it, for example a migration step. If you do need to write cache for the edited module, use:
